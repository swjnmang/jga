#!/usr/bin/env python3
"""Add manually curated, thematically appropriate distractors for all quote cards"""
import re

# Manually curated distractors - thematically matched, no special characters
quote_distractors = {
    ...
    'quote-tear-down-this-wall': ['John F. Kennedy, Ich bin ein Berliner', 'Winston Churchill, Iron Curtain Speech', 'Helmut Kohl, Blühende Landschaften'],
    ...
    'quote-ich-weiss-dass-ich-nichts-weiss': ['Platon, Hoehlengleichnis', 'Aristoteles, Zoon politikon', 'Descartes, Cogito ergo sum'],
...
for card_id, dists in quote_distractors.items():
    # Create distractor array - no special characters, already clean
...
print(f"✓ Added manually curated distractors to {count}/{len(quote_distractors)} quote cards")
if failed:
    print(f"⚠ Failed to match {len(failed)} cards: {failed[:5]}")
